...
[`ExpressWorkflowMaker`](https://github.com/MineralsCloud/ExpressWorkflowMaker.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("ExpressWorkflowMaker")
   ```

3. Run

   ```julia-repl
   julia> using ExpressWorkflowMaker
   ```
...
version of `ExpressWorkflowMaker`, type

```@repl
using Pkg
Pkg.update()
pkg"add ExpressWorkflowMaker#master"
```

in the second step above.

## Update `ExpressWorkflowMaker`
...
our [GitHub repository](https://github.com/MineralsCloud/ExpressWorkflowMaker.jl)
for new releases.
Once we release a new version, you can update `ExpressWorkflowMaker` by typing

```@repl
using Pkg
Pkg.update("ExpressWorkflowMaker")
Pkg.gc()
```

in Julia REPL.

## Uninstall and reinstall `ExpressWorkflowMaker`
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("ExpressWorkflowMaker")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall `ExpressWorkflowMaker`.
